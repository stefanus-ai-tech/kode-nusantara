...
const LOGO_URL = "https://images.unsplash.com/photo-1461749280684-dccba630e2f6?auto=format&fit=crop&w=60&q=80";
      ...
      <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <Link
              to="/"
              className="text-3xl sm:text-4xl font-extrabold tracking-tight bg-gradient-to-r from-[#a76d32] via-[#b88134] to-[#f8e5b6] bg-clip-text text-transparent drop-shadow"
            >
              KodeNusantara
            </Link>
            <span className="block text-xs ml-1 font-semibold text-[#905826] tracking-wide italic">
              #SemangatBatikCoding
            </span>
          </div>
        </div>
        <nav className="flex items-center gap-3">
          <Button
            ...
            className={`bg-[#b88134] hover:bg-[#85541a] text-white rounded-full px-6 py-2 shadow-lg text-base font-semibold transition-all
              border-2 border-[#aa782e] ${location.pathname==="/tanya-pertanyaan-baru" ? "ring-2 ring-[#a76d32]" : ""}`}
            size="lg"
